...
export default function OwnerDashboard() {
  ...
  return (
    <ScrollView className="flex-1 bg-dime-bg-grouped" contentContainerStyle={{ padding: 20, gap: 20 }}>
      <View>
        <Text className="text-[13px] text-dime-ink-3">Welcome back</Text>
        <Text className="text-[24px] font-bold text-dime-ink" style={{ letterSpacing: -1 }}>{restaurant?.name}</Text>
      </View>

      {restaurant?.status === "pending" ? (
        <View className="flex-row items-center gap-4 rounded-2xl border border-amber-300 bg-amber-50 p-5">
        ...
        <View className="flex-row items-center gap-4 rounded-2xl border border-red-300 bg-red-50 p-5">
      ...
      <Card>
        ...
        <Card.Body className="gap-2">
          {pending.slice(0, 5).map((o) => (
            <View key={o.id} className="flex-row items-center justify-between rounded-xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
              <View>
      ...
      <Card>
        ...
        <Card.Body className="gap-2">
          {lowStock.slice(0, 5).map((i) => (
            <View key={i.id} className="flex-row items-center justify-between rounded-xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
      ...
      <Card>
        ...
        <Card.Body className="gap-2">
          {(bookings ?? []).filter((b) => dayjs(b.date).isSame(dayjs(), "day")).slice(0, 5).map((b) => (
            <View key={b.id} className="flex-row items-center justify-between rounded-xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
              <View>
