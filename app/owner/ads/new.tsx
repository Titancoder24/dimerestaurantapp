    ...
    <Screen scroll={false}>
      ...
      <View className="px-4">
        <View className="h-1.5 overflow-hidden rounded-full bg-dime-bg-2">
          <View className="h-full rounded-full bg-dime-orange-500" style={{ width: `${((step + 1) / 3) * 100}%` }} />
        </View>
      </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 40, gap: 16 }} keyboardShouldPersistTaps="handled">
            ...
            <View className="flex-row flex-wrap gap-2">
              {POSTER_TEMPLATES.map((t) => (
                <Pressable
                  ...
                  className={`w-[31%] rounded-xl border-2 p-1 ${templateId === t.id ? "border-dime-orange-500" : "border-transparent"}`}
                >
                  ...
                  <Text className="mt-1 text-center text-[10px] font-semibold text-dime-ink">{t.name}</Text>
                </Pressable>
              ))}
            </View>
          </View>
            ...
            >
              {imageUrl ? (
                ...
                <View className="items-center">
                  <Icon name="photo.fill" size={26} color="#FC8019" />
                  <Text className="mt-1 text-[12px] font-semibold text-dime-orange-700">Tap to upload (16:9)</Text>
                </View>
              )}
            </Pressable>
        ...
        {step === 2 ? (
          <View className="gap-4">
            ...
            <Text className="text-[13px] font-medium text-dime-ink-2">Where should it show?</Text>
            <View className="gap-2">
              {placements.map((p) => (
                <Pressable
                  key={p.k}
                  onPress={() => { haptic.select(); setPlacement(p.k); }}
                  className={`flex-row items-center gap-3 rounded-xl border p-3 ${placement === p.k ? "border-dime-orange-500 bg-dime-orange-50" : "border-dime-border bg-white"}`}
                >
                  <View className={`h-4 w-4 rounded-full border-2 ${placement === p.k ? "border-dime-orange-500 bg-dime-orange-500" : "border-dime-ink-3"}`} />
                  <View className="flex-1">
                    <Text className="text-[14px] font-semibold text-dime-ink">{p.l}</Text>
            ...
            <Text className="mt-2 text-[13px] font-medium text-dime-ink-2">Daily budget</Text>
            <ChipRow>
              {dailyBudgets.map((b) => (
            ...
            <Text className="mt-2 text-[13px] font-medium text-dime-ink-2">Duration</Text>
            <ChipRow>
              {durations.map((d) => (
            ...
            <View className="rounded-2xl bg-dime-orange-50 p-4">
              <View className="flex-row items-center justify-between">
                <Text className="text-[13px] font-semibold text-dime-orange-700">Total cost</Text>
                <Text className="text-[24px] font-bold text-dime-orange-700">{rupees(totalCost)}</Text>
              </View>
              <Text className="text-[11px] text-dime-orange-700">
                {rupees(dailyBudget)} × {duration} days · live for ~{Math.round(duration * 24)} hrs after approval
              </Text>
              <Text className="mt-2 text-[11px] text-dime-orange-700 opacity-80">
