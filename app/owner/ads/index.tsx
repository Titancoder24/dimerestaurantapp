    ...
    <Screen scroll={false}>
      <Header
        ...
        right={
          ...
          >
            <Text className="text-[12px] font-bold text-white">+ Create ad</Text>
          </Pressable>
        }
      />

      <View className="flex-row gap-2 px-5">
        <Tile color="bg-dime-primary-500" label="Impressions" value={totals.impressions.toLocaleString()} />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center py-16">
            <Icon name="sparkles" size={32} color="#BFBFBF" />
            <Text className="mt-3 text-[16px] font-bold text-dime-ink">No ads yet</Text>
        ...
        renderItem={({ item: ad }) => (
          <Pressable className="rounded-2xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
            <PosterCard
            ...
            <View className="mt-4 flex-row items-center gap-2">
              <Text className="flex-1 text-[15px] font-bold text-dime-ink" numberOfLines={1}>{ad.title}</Text>
            ...
            {ad.status === "rejected" && ad.admin_notes ? (
              <View className="mt-2 rounded-md bg-red-50 p-2">
                <Text className="text-[11px] font-bold text-dime-danger">Rejected</Text>
