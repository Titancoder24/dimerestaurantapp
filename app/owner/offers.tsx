  ...
  return (
    <Screen>
      ...
      <View className="mx-4 gap-3">
        {(offers ?? []).map((o) => (
          <View key={o.id} className="rounded-2xl border border-dime-border bg-white p-4">
            <View className="flex-row items-center justify-between">
              <Text className="text-[11px] font-bold uppercase tracking-widest text-dime-orange-700">{o.promo_code ?? "PROMO"}</Text>
              <Switch value={o.is_active} onValueChange={() => toggle(o.id, o.is_active)} trackColor={{ true: "#FC8019", false: "#D1D1D6" }} />
            </View>
