...
export default function OwnerOnboarding() {
  ...
  useEffect(() => {
    if (!profile) requestAnimationFrame(() => router.replace("/login"));
  }, [profile, router]);
    ...
    <Screen scroll={false}>
      ...
      <View className="px-5">
        <View className="h-1.5 overflow-hidden rounded-full bg-dime-bg-2">
          <View className="h-full rounded-full bg-dime-primary-500" style={{ width: `${progress}%` }} />
        </View>
      </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, paddingBottom: 40, gap: 20 }} keyboardShouldPersistTaps="handled">
        {step === 0 ? (
          <View className="gap-4">
            ...
            <View>
              <Text className="mb-2 text-[13px] font-bold text-dime-ink-2">Type</Text>
              <ChipRow>
            ...
            <View>
              <Text className="mb-2 text-[13px] font-bold text-dime-ink-2">Cuisines (pick all that apply)</Text>
              <ChipRow>
                {cuisinePool.map((c) => (
            ...
            <View>
              <Text className="mb-2 text-[13px] font-bold text-dime-ink-2">Price range</Text>
              <ChipRow>
                {[1, 2, 3, 4].map((p) => (
        ...
        {step === 4 ? (
          <View className="gap-4">
            ...
            {dayKeys.map((k, i) => (
              <View key={k} className="flex-row items-center gap-4 rounded-xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
                <View className="w-10">
                  <Text className="text-[13px] font-bold text-dime-ink">{days[i]}</Text>
                </View>
                <Pressable
                  onPress={() => setHour(k, "closed", !hours[k]!.closed)}
                  className={`rounded-full px-3 py-1 ${hours[k]!.closed ? "bg-gray-200" : "bg-emerald-100"}`}
                >
                  <Text className={`text-[11px] font-bold ${hours[k]!.closed ? "text-gray-600" : "text-emerald-700"}`}>
        ...
        {step === 6 ? (
          <View className="gap-4">
            ...
            <View>
              <Text className="mb-2 text-[13px] font-bold text-dime-ink-2">Cover photo</Text>
              <Pressable
                onPress={uploadCover}
                disabled={uploading}
              ...
              >
                {coverUrl ? (
                  ...
                  <View className="items-center">
                    <Icon name="photo.fill" size={28} color="#FF6B2C" />
                    <Text className="mt-2 text-[13px] font-bold text-dime-primary-700">Tap to upload</Text>
            ...
            <View>
              <View className="mb-2 flex-row items-center justify-between">
                <Text className="text-[13px] font-bold text-dime-ink-2">Gallery ({gallery.length}/8)</Text>
                <Pressable onPress={uploadGallery} disabled={uploading}>
                  <Text className="text-[12px] font-bold text-dime-primary-600">+ Add photos</Text>
                </Pressable>
              </View>
...
function SectionTitle({ icon, title }: { icon: string; title: string }) {
  return (
    <View className="flex-row items-center gap-2">
      <View className="h-8 w-8 items-center justify-center rounded-full bg-dime-primary-50">
        <Icon name={icon} size={16} color="#FF6B2C" />
      </View>
      <Text className="text-[18px] font-bold text-dime-ink" style={{ letterSpacing: -0.5 }}>{title}</Text>
    </View>
  );
}
