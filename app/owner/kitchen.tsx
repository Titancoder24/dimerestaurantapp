  ...
  return (
    <Screen scroll={false}>
      ...
      <FlatList
        ...
        renderItem={({ item: o }) => {
          const target = Math.max(...o.order_items.map((i) => 15));
            ...
            <View className="rounded-2xl border border-dime-border bg-white p-4">
              <View className="flex-row items-center justify-between">
                <View className="flex-row items-center gap-3">
                  <View className="h-10 w-10 items-center justify-center rounded-full bg-dime-orange-100">
                    <Text className="text-[14px] font-bold text-dime-orange-700">{o.table_id ? "T" : "TA"}</Text>
                  </View>
                  <View>
                    <Text className="text-[15px] font-semibold text-dime-ink">{o.order_number}</Text>
              ...
              <View className="mt-3 gap-1 border-t border-dime-border pt-3">
                {o.order_items.map((it) => (
                  <View key={it.id} className="flex-row items-start gap-2">
                    <Text className="w-7 text-[14px] font-bold text-dime-orange-600">{it.quantity}×</Text>
              ...
              <View className="mt-3 flex-row items-center justify-between border-t border-dime-border pt-3">
                ...
                {o.status === "received" ? (
                  ...
                  <Text className="text-[12px] text-emerald-600 font-semibold">Waiting for pickup</Text>
                )}
              </View>
            </View>
          );
        ...
        ListEmptyComponent={
          <View className="items-center py-16">
            ...
            <Text className="mt-3 text-[15px] font-semibold text-dime-ink">All clear</Text>
