  ...
  return (
    <Screen scroll={false}>
      <Header
        ...
        right={
          <Pressable onPress={() => setComposing(true)} className="rounded-full bg-dime-orange-500 px-3 py-1.5">
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center py-16">
            <Icon name="tray.fill" size={32} color="#C7C7CC" />
            <Text className="mt-3 text-[15px] font-semibold text-dime-ink">No tickets yet</Text>
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            className="rounded-2xl border border-dime-border bg-white p-3"
          >
            <View className="flex-row items-center gap-2">
              <Text className="text-[11px] font-bold uppercase tracking-widest text-dime-ink-3">{item.ticket_number}</Text>
            ...
            <Text className="mt-1 text-[14px] font-semibold text-dime-ink">{item.subject}</Text>
...
function ComposeSheet({
  ...
  return (
    <Sheet visible={visible} onClose={onClose} maxHeight="92%">
      <Sheet.Body>
        <Text className="text-[18px] font-semibold text-dime-ink">New ticket</Text>
          ...
          <View className="flex-row flex-wrap gap-2">
            {categories.map((c) => (
              <Pressable
                ...
                className={`flex-row items-center gap-1.5 rounded-full border px-3 py-2 ${category === c.k ? "border-dime-orange-500 bg-dime-orange-50" : "border-dime-border bg-white"}`}
              >
                <Icon name={c.icon} size={12} color={category === c.k ? "#FC8019" : "#8E8E93"} />
                <Text className={`text-[12px] font-medium ${category === c.k ? "text-dime-orange-700" : "text-dime-ink-2"}`}>{c.l}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View className="mt-4">
          <Text className="mb-2 text-[13px] font-medium text-dime-ink-2">Priority</Text>
          <ChipRow>
            {priorities.map((p) => (
        ...
        <View className="mt-4 gap-3">
          ...
          <View>
            <Text className="mb-1.5 text-[13px] font-medium text-dime-ink-2">What happened?</Text>
            <TextInput
              value={message}
              ...
              className="min-h-[140px] rounded-xl border border-dime-border bg-white p-3 text-[14px] text-dime-ink"
...
function ThreadSheet({
  ...
  return (
    <Sheet visible={!!ticket} onClose={onClose} maxHeight="92%">
      <Sheet.Body>
        <View className="flex-row items-center gap-2">
          <Text className="text-[11px] font-bold uppercase tracking-widest text-dime-ink-3">{ticket.ticket_number}</Text>
        ...
        <Text className="mt-1 text-[18px] font-semibold text-dime-ink">{ticket.subject}</Text>
          ...
          {ticket.support_messages.map((m) => {
            ...
            return (
              <View key={m.id} className={mine ? "items-end" : "items-start"}>
                <View className={`max-w-[80%] rounded-2xl p-3 ${mine ? "bg-dime-orange-500" : "bg-dime-bg-2"}`}>
        ...
        {ticket.status !== "closed" ? (
          <View className="mt-4 flex-row items-end gap-2">
            <TextInput
              ...
              placeholderTextColor="#8E8E93"
              multiline
              className="min-h-[44px] flex-1 rounded-xl border border-dime-border bg-white p-3 text-[14px] text-dime-ink"
            />
