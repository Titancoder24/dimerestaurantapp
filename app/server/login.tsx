  ...
  async function submit() {
    ...
    try {
      const { data, error } = await supabase
        .from("staff")
        .select("id,restaurant_id,name,role,permissions")
        .eq("pin", pin)
        .eq("is_active", true)
        .limit(1);
  ...
  return (
    <Screen scroll={false}>
      <View className="flex-1 items-center justify-center p-6">
        ...
        <Text className="text-[24px] font-semibold text-dime-ink">Server Panel</Text>
        ...
        <View className="mt-8 flex-row gap-3">
          {[0, 1, 2, 3].map((i) => (
            <View
              key={i}
              className={`h-4 w-4 rounded-full ${pin.length > i ? "bg-dime-orange-500" : "bg-dime-border"}`}
            />
          ))}
        </View>
        ...
        <Pressable onPress={() => router.replace("/")} className="mt-6">
          <Text className="text-[13px] text-dime-ink-3">Not a staff member? <Text className="font-semibold text-dime-orange-600">Back</Text></Text>
        </Pressable>
      </View>
    </Screen>
  );
