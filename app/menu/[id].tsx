    ...
    <Screen scroll={false}>
      ...
      <View className="px-4">
        <Input
          ...
          leading={<Icon name="magnifyingglass" size={16} color="#8E8E93" />}
        />
      ...
      <FlatList
        ...
        renderItem={({ item: group }) => (
          <View>
            <Text className="mb-2 text-[18px] font-semibold text-dime-ink">
              {group.category.name} <Text className="text-[13px] text-dime-ink-3">({group.items.length})</Text>
            </Text>
      ...
      {cartCount > 0 ? (
        <Pressable
          ...
          className="absolute inset-x-4 bottom-6 flex-row items-center gap-3 rounded-full bg-dime-orange-500 px-5 py-4"
          style={{ shadowColor: "#FC8019", shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.35, shadowRadius: 12 }}
        >
          <View className="h-7 w-7 items-center justify-center rounded-full bg-white">
            <Text className="text-[13px] font-bold text-dime-orange-600">{cartCount}</Text>
          </View>
          <Text className="flex-1 text-[15px] font-semibold text-white">
            {cartCount} item{cartCount > 1 ? "s" : ""} · {rupees(cartSubtotal)}
          </Text>
          <Text className="text-[14px] font-semibold text-white">View cart</Text>
...
function MenuItemRow({
  ...
  return (
    <Pressable
      onPress={onDetails}
      className="flex-row items-start gap-3 rounded-2xl border border-dime-border bg-white p-3"
    >
      <View className="flex-1">
        ...
        <Text className="mt-1 text-[15px] font-semibold text-dime-ink">{item.name}</Text>
        <Text className="text-[13px] font-semibold text-dime-ink-2">{rupees(item.price)}</Text>
        {item.description ? (
      ...
      <View className="relative">
        <Image source={{ uri: item.images[0] ?? "" }} className="h-24 w-24 rounded-xl" />
        <Pressable
          onPress={(e) => { e.stopPropagation(); onAdd(item); }}
          className="absolute -bottom-3 left-1/2 -translate-x-1/2 rounded-full border border-dime-orange-500 bg-white px-5 py-1.5"
        >
          <Text className="text-[13px] font-bold text-dime-orange-600">ADD</Text>
        </Pressable>
      </View>
    </Pressable>
  );
...
function ItemDetailSheet({
  ...
  return (
    <Sheet visible={!!item} onClose={onClose} maxHeight="85%">
      <Sheet.Body>
        <Image source={{ uri: item.images[0] ?? "" }} className="-mx-5 h-48" />
        <View className="mt-4 flex-row items-center gap-2">
          <VegDot veg={item.is_veg} />
          {item.is_bestseller ? <Badge tone="gold" label="Bestseller" /> : null}
        </View>
        <Text className="mt-2 text-[22px] font-semibold text-dime-ink">{item.name}</Text>
        <Text className="text-[16px] font-semibold text-dime-ink-2">{rupees(item.price)}</Text>
        {item.description ? <Text className="mt-2 text-[14px] text-dime-ink-2">{item.description}</Text> : null}

        <View className="mt-4 flex-row gap-3">
          {item.calories ? (
            <View className="rounded-full bg-dime-bg-2 px-3 py-1.5">
              <Text className="text-[12px] text-dime-ink-2">{item.calories} cal</Text>
            </View>
          ) : null}
          <View className="rounded-full bg-dime-bg-2 px-3 py-1.5">
            <Text className="text-[12px] text-dime-ink-2">{item.prep_time_minutes} min</Text>
          </View>
        </View>

        {item.allergens.length > 0 ? (
          <View className="mt-4 rounded-xl bg-amber-50 p-3">
            <Text className="text-[12px] font-semibold uppercase tracking-widest text-amber-800">Allergens</Text>
            <Text className="mt-0.5 text-[13px] text-amber-900">{item.allergens.join(", ")}</Text>
          </View>
        ) : null}
