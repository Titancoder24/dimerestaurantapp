  ...
  return (
    <Screen scroll={false}>
      <Header
        ...
        right={
          <Pressable onPress={() => setInviting(true)} className="rounded-full bg-dime-orange-500 px-3 py-1.5">
            <Text className="text-[12px] font-semibold text-white">+ Invite</Text>
          </Pressable>
        }
      />

      <View className="mx-4 rounded-2xl border border-dime-border bg-white p-3">
        <Text className="text-[11px] font-bold uppercase tracking-widest text-dime-ink-3">Departments</Text>
        <View className="mt-2 flex-row flex-wrap gap-2">
          {Object.entries(roleMeta).map(([k, m]) => (
            <View key={k} className="rounded-full bg-dime-bg-2 px-3 py-1.5">
              <Text className="text-[11px] font-semibold text-dime-ink-2">{m.label} <Text className="text-dime-orange-600">{counts[k] ?? 0}</Text></Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Pressable onPress={() => setEditing(item)} className="flex-row items-center gap-3 rounded-2xl border border-dime-border bg-white p-3">
        ...
        ListEmptyComponent={
          <View className="items-center py-16">
            <Icon name="person.fill" size={28} color="#C7C7CC" />
            <Text className="mt-3 text-[15px] font-semibold text-dime-ink">No team yet</Text>
...
function InviteSheet({ visible, onClose, onInvited }: { visible: boolean; onClose: () => void; onInvited: () => void }) {
  ...
  return (
    <Sheet visible={visible} onClose={onClose} maxHeight="80%">
      <Sheet.Body>
        <Text className="text-[18px] font-semibold text-dime-ink">Invite team member</Text>
        ...
        <View className="mt-4 gap-3">
          ...
          <View>
            <Text className="mb-2 text-[13px] font-medium text-dime-ink-2">Department</Text>
              ...
              {(Object.keys(roleMeta) as AdminRole[]).map((k) => {
                ...
                return (
                  <Pressable
                    ...
                    className={`flex-row items-center gap-3 rounded-xl border p-3 ${selected ? "border-dime-orange-500 bg-dime-orange-50" : "border-dime-border bg-white"}`}
                  >
                    <Badge tone={m.tone} label={m.label} />
                    <Text className="flex-1 text-[12px] text-dime-ink-2">{m.description}</Text>
                    {selected ? <Icon name="checkmark.circle.fill" size={16} color="#FC8019" /> : null}
                  </Pressable>
                );
              })}
            </View>
...
function EditSheet({ member, onClose, onSaved }: { member: TeamMember | null; onClose: () => void; onSaved: () => void }) {
  ...
  async function suspend() {
    Alert.alert("Suspend member?", "They'll lose access immediately.", [
    ...
    <Sheet visible={!!member} onClose={onClose} maxHeight="92%">
      <Sheet.Body>
        <View className="flex-row items-center gap-3">
          ...
          <View className="flex-1">
            <Text className="text-[16px] font-semibold text-dime-ink">{member.name ?? "—"}</Text>
            <Text className="text-[12px] text-dime-ink-3">{member.email}</Text>
          </View>
        </View>

        <View className="mt-4">
          <Text className="mb-2 text-[13px] font-medium text-dime-ink-2">Role</Text>
          <ChipRow>
            {(Object.keys(roleMeta) as AdminRole[]).map((k) => (
        ...
        <View className="mt-4">
          <Text className="mb-2 text-[13px] font-medium text-dime-ink-2">Permissions</Text>
          <View className="rounded-2xl border border-dime-border bg-white">
            {allPermissions.map((p, i) => (
              <View key={p} className={`flex-row items-center justify-between p-3 ${i > 0 ? "border-t border-dime-border" : ""}`}>
