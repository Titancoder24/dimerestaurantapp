  ...
  return (
    <Screen scroll={false}>
      ...
      <FlatList
        ...
        renderItem={({ item: t }) => (
          <View className="rounded-xl border border-dime-border bg-white p-3">
            ...
            <View className="mt-2 flex-row items-center gap-2">
              ...
              <View className="ml-auto flex-row gap-2">
                {t.status === "open" ? (
                  <Pressable onPress={() => setStatus(t.id, "in_progress")} className="rounded-full bg-dime-orange-500 px-3 py-1">
                    <Text className="text-[11px] font-semibold text-white">Take</Text>
                  </Pressable>
                ) : null}
                {t.status === "in_progress" ? (
                  <Pressable onPress={() => setStatus(t.id, "resolved")} className="rounded-full bg-emerald-500 px-3 py-1">
                    <Text className="text-[11px] font-semibold text-white">Resolve</Text>
                  </Pressable>
                ) : null}
              </View>
            </View>
