    ...
    <Screen scroll={false}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <View className="flex-row items-center gap-3 rounded-2xl border border-dime-border bg-white p-3">
            ...
            <View className="flex-1">
              <Text className="text-[14px] font-semibold text-dime-ink">{item.restaurant.name}</Text>
              ...
              <View className="mt-1 flex-row gap-1.5">
                <View className="flex-row items-center gap-1 rounded-md bg-green-50 px-1.5 py-0.5">
                  ...
                  <Text className="text-[10px] font-semibold text-green-700">{Number(item.restaurant.rating).toFixed(1)}</Text>
                </View>
                <Badge tone="gray" label={`${item.orders} orders`} />
              </View>
            </View>
            <View className="items-end">
              {metric === "gmv" ? <Text className="text-[15px] font-bold text-dime-ink">{rupees(item.gmv)}</Text> : null}
              {metric === "rating" ? <Text className="text-[15px] font-bold text-dime-ink">{Number(item.restaurant.rating).toFixed(2)} ★</Text> : null}
              {metric === "orders" ? <Text className="text-[15px] font-bold text-dime-ink">{item.orders}</Text> : null}
              {metric === "growth" ? (
                <Text className={`text-[15px] font-bold ${item.growth >= 0 ? "text-emerald-600" : "text-dime-danger"}`}>
        ...
        ListEmptyComponent={<Text className="px-4 py-12 text-center text-[13px] text-dime-ink-3">No data.</Text>}
      />
    </Screen>
  );
}
