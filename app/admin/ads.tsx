  ...
  return (
    <Screen scroll={false}>
      ...
      <View className="flex-row gap-2 px-4">
        ...
        <Tile color="bg-dime-orange-500" label="Ad revenue" value={rupees(counts.revenue)} />
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center py-12">
            ...
            <Text className="mt-2 text-[14px] font-semibold text-dime-ink">Nothing to review</Text>
            <Text className="text-[12px] text-dime-ink-3">All caught up.</Text>
          </View>
        }
        renderItem={({ item: ad }) => (
          <Pressable onPress={() => setReviewing(ad)} className="rounded-2xl border border-dime-border bg-white p-3">
            <PosterCard
            ...
            <View className="mt-3 flex-row items-center gap-2">
              <Text className="flex-1 text-[14px] font-semibold text-dime-ink" numberOfLines={1}>{ad.title}</Text>
  ...
  return (
    <Sheet visible={!!ad} onClose={onClose} maxHeight="92%">
      <Sheet.Body>
        ...
        <View className="mt-4">
          <Text className="text-[18px] font-semibold text-dime-ink">{ad.title}</Text>
          <Text className="text-[12px] text-dime-ink-3">
            {ad.restaurants?.name} · {ad.restaurants?.city} · submitted {timeAgo(ad.created_at)}
          </Text>
        </View>

        <View className="mt-4 rounded-2xl bg-dime-bg-2 p-3">
          <View className="flex-row items-center justify-between"><Text className="text-[12px] text-dime-ink-2">Placement</Text><Text className="text-[13px] font-semibold text-dime-ink">{ad.placement.replace("_", " ")}</Text></View>
          <View className="mt-2 flex-row items-center justify-between"><Text className="text-[12px] text-dime-ink-2">Daily budget</Text><Text className="text-[13px] font-semibold text-dime-ink">{rupees(ad.daily_budget)}</Text></View>
          <View className="mt-2 flex-row items-center justify-between"><Text className="text-[12px] text-dime-ink-2">Duration</Text><Text className="text-[13px] font-semibold text-dime-ink">{ad.duration_days} days</Text></View>
          <View className="mt-2 border-t border-dime-border pt-2 flex-row items-center justify-between">
            <Text className="text-[14px] font-semibold text-dime-ink">Total revenue</Text>
            <Text className="text-[18px] font-bold text-dime-orange-600">{rupees(Number(ad.total_cost))}</Text>
          </View>
        </View>
        ...
        {(ad.status === "active" || ad.status === "completed") ? (
          <View className="mt-4 flex-row gap-2">
            <View className="flex-1 rounded-2xl bg-emerald-50 p-3">
              <Text className="text-[10px] font-bold uppercase text-emerald-700">Impressions</Text>
              <Text className="text-[20px] font-bold text-emerald-900">{ad.impressions.toLocaleString()}</Text>
            </View>
            <View className="flex-1 rounded-2xl bg-blue-50 p-3">
              <Text className="text-[10px] font-bold uppercase text-blue-700">Clicks</Text>
              <Text className="text-[20px] font-bold text-blue-900">{ad.clicks.toLocaleString()}</Text>
            </View>
            <View className="flex-1 rounded-2xl bg-amber-50 p-3">
              <Text className="text-[10px] font-bold uppercase text-amber-700">CTR</Text>
              <Text className="text-[20px] font-bold text-amber-900">
        ...
        {ad.status === "pending_review" ? (
          <>
            <View className="mt-4">
              <Text className="mb-2 text-[12px] font-medium text-dime-ink-2">Rejection reason (only used if you reject)</Text>
              <TextInput
                value={reason} onChangeText={setReason} multiline placeholder="e.g. Image violates content policy"
                placeholderTextColor="#8E8E93"
                className="min-h-[80px] rounded-xl border border-dime-border bg-white p-3 text-[14px] text-dime-ink"
