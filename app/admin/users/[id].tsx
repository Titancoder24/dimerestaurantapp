...
export default function AdminUserDetail() {
  ...
  return (
    <Screen>
      ...
      <View className="mx-5 overflow-hidden rounded-2xl bg-dime-ink" style={{ backgroundColor: "#1C1C1E" }}>
        <View className="p-5">
          <View className="flex-row items-center gap-4">
            <Avatar name={user.name ?? user.email} uri={user.avatar_url} size={56} />
            <View className="flex-1">
              <Text className="text-[18px] font-bold text-white" style={{ letterSpacing: -0.5 }}>{user.name ?? "—"}</Text>
        ...
        <Section title="Complaints & tickets" icon="tray.fill">
          <View className="gap-2">
            {tickets.slice(0, 5).map((t) => (
              <View key={t.id} className="rounded-2xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
                <View className="flex-row items-center gap-2">
                  <Text className="text-[11px] font-bold uppercase text-dime-ink-4" style={{ letterSpacing: 1.5 }}>{t.ticket_number}</Text>
                ...
                <Text className="mt-1 text-[14px] font-bold text-dime-ink">{t.subject}</Text>
      ...
      <Section title="Bookings" icon="calendar">
        <View className="gap-2">
          {(bookings ?? []).slice(0, 8).map((b) => (
            <View key={b.id} className="flex-row items-center gap-4 rounded-2xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
              <View className="flex-1">
                <Text className="text-[13px] font-bold text-dime-ink">{b.restaurants?.name ?? "—"}</Text>
      ...
      <Section title="Orders" icon="bag.fill">
        <View className="gap-2">
          {(orders ?? []).slice(0, 8).map((o) => (
            <View key={o.id} className="flex-row items-center gap-4 rounded-2xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
              <View className="flex-1">
      ...
      <Section title="Reviews" icon="star.fill">
        <View className="gap-2">
          {(reviews ?? []).slice(0, 6).map((r) => (
            <View key={r.id} className="rounded-2xl bg-white p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 2 }}>
              <View className="flex-row items-center justify-between">
                <Text className="text-[13px] font-bold text-dime-ink">{r.restaurants?.name ?? "—"}</Text>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <View className="items-center">
      <Text className="text-[16px] font-bold text-white">{value}</Text>
      <Text className="text-[10px] uppercase text-white/60" style={{ letterSpacing: 1.5 }}>{label}</Text>
    </View>
  );
}
