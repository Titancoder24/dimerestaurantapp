                ...
                <View className="max-w-[380px]">
                  ...
                  <View className="mt-8 flex-row gap-4">
                    {[
                      ...
                      <View key={f.label} className="items-center rounded-xl bg-white/10 px-4 py-3">
                        <Icon name={f.icon} size={18} color="#FF6B2C" />
                        <Text className="mt-1 text-[10px] font-bold text-white/70">{f.label}</Text>
                      </View>
                    ))}
                  </View>
                </View>
            ...
            <View className={`flex-1 justify-center px-6 py-10 ${wide ? "px-16" : ""}`}>
              <View style={{ maxWidth: 420, width: "100%", alignSelf: "center" }}>
                ...
                <View className="gap-4">
                  ...
                  <View>
                    ...
                    <Pressable onPress={() => setShowForgot(true)} className="mt-1.5 self-end">
                      <Text className="text-[12px] font-bold text-dime-primary-500">Forgot password?</Text>
                    </Pressable>
                  </View>
                </View>
                ...
                <View className="mt-5 flex-row items-center justify-center gap-1">
                  <Text className="text-[14px] text-dime-ink-3">New to DIME?</Text>
                  <Link href="/signup" className="text-[14px] font-bold text-dime-primary-500">
                    Create an account
                  </Link>
                </View>

                <View className="mt-2 items-center">
                  <Link href={{ pathname: "/signup", params: { role: "owner" } }} className="text-[13px] text-dime-ink-3">
                    Restaurant owner?{" "}
                    <Text className="font-bold text-dime-primary-500">Partner with DIME</Text>
                  </Link>
                </View>
                ...
                <View className="mb-4 mt-8 flex-row items-center gap-4">
                  ...
                  <Text className="text-[10px] font-bold uppercase text-dime-ink-4" style={{ letterSpacing: 2 }}>
                    Quick demo
                  </Text>
                  <View className="h-px flex-1 bg-neutral-100" />
                </View>
                ...
                <View className="flex-row gap-3 pb-4">
                  {[
                    ...
                    <Pressable
                      ...
                      className="flex-1 items-center rounded-2xl border border-neutral-100 bg-white px-3 py-4"
                      style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.03, shadowRadius: 8, elevation: 1 }}
                    >
                      <View className="mb-2 h-9 w-9 items-center justify-center rounded-full bg-dime-bg-2">
                        <Icon name={d.icon} size={14} color="#3A3A3C" />
                      </View>
                      <Text className="text-[13px] font-bold text-dime-ink">{d.label}</Text>
                      <Text className="mt-0.5 text-[10px] text-dime-ink-3">{d.sub}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
      ...
      <Sheet.Body>
        {sent ? (
          <View className="items-center py-6">
            ...
            <Text className="text-[18px] font-bold text-dime-ink" style={{ letterSpacing: -0.5 }}>Check your email</Text>
            <Text className="mt-2 text-center text-[13px] text-dime-ink-3">
          ...
          <>
            <Text className="text-[18px] font-bold text-dime-ink" style={{ letterSpacing: -0.5 }}>Reset password</Text>
